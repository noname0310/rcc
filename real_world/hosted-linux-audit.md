...
The checked-in wrapper downloads the official amalgamation into this project's
ignored `upstream/` tree, compiles the core and CLI shell as separate objects
with `rcc --linux-gnu-hosted --std=c11`, links them with host `cc -ldl -lm`, and
runs an in-memory `CREATE TABLE`/`INSERT`/`SELECT` smoke that prints `1`. The
CLI probe deliberately does not define
...
bash scripts/run-true-probe.sh
```
...
Current `src/true.c` compiler-owned blockers:
...
- Generated gnulib/coreutils probes: `tasks/16-linux-glibc-compat/13-gnulib-config-header-probe.md`
  through `24-coreutils-true-runtime-oracle.md`
