...
| GNU coreutils `src/true` | PASS | BLOCKED | TODO | BLOCKED | BLOCKED | `tasks/16-linux-glibc-compat/21-gnu-include-next-directive.md` |
...
| Syntax/HIR | BLOCKED | `logs/true-probe/rcc-true.stderr` reports GNU `#include_next` first in generated replacement headers. | `tasks/16-linux-glibc-compat/21-gnu-include-next-directive.md` |
| Object | TODO | Not attempted while Syntax/HIR is blocked. | `16-21`, then `16-22`, then `16-23` |
...
1. `tasks/16-linux-glibc-compat/21-gnu-include-next-directive.md`
2. `tasks/16-linux-glibc-compat/22-gnulib-funcdecl-macro-surface.md`
3. `tasks/16-linux-glibc-compat/23-coreutils-posix-declaration-sweep.md`
4. `tasks/16-linux-glibc-compat/24-coreutils-true-runtime-oracle.md`
...
but phase 16 stays open because tasks 16-21 through 16-24 are pending.
