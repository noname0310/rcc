...
The release dashboard remains strict-C99 first, but C11 is now tracked as a
separate core gate instead of being mixed into C99 suite percentages.

| Target bucket | Current gate | Status | Deferred / optional pieces |
| --- | --- | --- | --- |
| Strict C99 | `c-testsuite`, chibicc `stage-1-3`, tcc-tests2 release subset, llvm-test-suite subset | required release rows are green in the autogenerated suite table | xfails remain explicit non-C99/TinyCC/extension rows |
| C11 core language | parser/HIR/typeck unit coverage plus LLVM e2e `c11_core_constructs_compile_link_and_run` | `_Static_assert`, `_Alignof`/`_Alignas`, `_Generic`, `_Noreturn`, `_Atomic`, `_Thread_local`, anonymous records, Unicode literals, and resource C11 headers have focused gates | full external C11 conformance suite has not been declared |
| C11 hosted library surface | `rcc_driver::hosted_linux_header_gate_lowers_representative_fixtures` on Linux | `stdalign.h`, `stdnoreturn.h`, `stdatomic.h`, `threads.h`, `uchar.h`, and C11 deltas in `assert.h`, `float.h`, `stdlib.h`, and `time.h` lower under `-std=c11` | Annex K, analyzability, target-generated float tables, fully conforming generic atomic operations, and a pthread-independent C11 threads runtime |
